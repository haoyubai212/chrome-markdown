...
- 单文件模式默认显示文档大纲；点击“文件”会直接请求恢复已记住的目录，或为新项目打开一次目录选择器
...
首次从 Chrome 直接打开一个新项目中的 Markdown 时，左侧默认显示大纲。点击“文件”会立即打开系统目录选择器；选择当前项目目录一次后，以后打开同一项目的文件会自动匹配，并在需要时请求恢复访问。不同项目的授权互不覆盖。
...
npm test
npm run build
npm run package      # 生成 local-md-reader-0.1.7.zip
```
